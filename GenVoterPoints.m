%function [xyrandMat, votersNoHome] = GenVoterPoints(RC, mu)
%% 0. Plot
...
%% Find points satisfying lastRanked above NotRanked
numFullVotes = sum(~~RC,2);

lastAboveNot = ones(n1, length(d));

numUniqueVotes = length(unique(RC, 'rows'));


%NOTE: MORE EFFICIENT BY SKIPPING K LOOP WHEN FIRST 0 APPEARS
%ALSO SKIP I LOOP WHEN REPEATED VOTE APPEARS USING SORT
for i = 1:n1
